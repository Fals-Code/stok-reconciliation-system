...
language plpgsql
security definer
set search_path = pg_catalog, auth, app, catalog, inventory, operations, reconciliation, extensions
as $$
declare
  v_scope constant text := 'RUN_RECONCILIATION';
  v_rule_set_version constant text := 'core-integrity-v8';
...
begin
  ...
  lock table inventory.stock_reservations
    in share mode;

  lock table inventory.stock_transactions
    in share mode;

  lock table operations.marketplace_orders
    in share mode;

  lock table operations.marketplace_events
    in share mode;

  lock table operations.marketplace_event_lines
    in share mode;

  lock table operations.marketplace_ship_allocations
    in share mode;

  lock table operations.returns
    in share mode;

  lock table operations.return_items
    in share mode;

  lock table operations.return_events
    in share mode;

  lock table operations.return_event_lines
    in share mode;

  lock table operations.return_receipts
    in share mode;

  lock table operations.return_receipt_lines
    in share mode;

  lock table operations.return_inspections
    in share mode;

  lock table operations.return_inspection_allocations
    in share mode;

  lock table operations.receipts
    in share mode;

  lock table operations.manual_outbounds
    in share mode;
  ...
  loop
    ...
    if v_check_code = 'LEDGER_BATCH_PROJECTION' then
    ...
    elsif v_check_code = 'RESERVATION_CONSISTENCY' then
      with reservation_products as (
      ...
      loop
        ...
        values (
          ...
          'OPEN',
          case
            when v_mismatch.issue_code in (
              'RESERVATION_EXCEEDS_SELLABLE',
              'PROJECTION_RESERVED_EXCEEDS_SELLABLE'
            ) then 'CRITICAL'
            else 'HIGH'
          end,
          'PRODUCT_RESERVATION',
          ...
          jsonb_build_object(
            'reservedQty',
            v_mismatch.expected_reserved_qty,
            'sellableQty',
            v_mismatch.sellable_qty,
            'availableQty',
            v_mismatch.expected_available_qty
          ),
          jsonb_build_object(
            'reservedQty',
            v_mismatch.actual_reserved_qty,
            'sellableQty',
            v_mismatch.sellable_qty,
            'availableQty',
            v_mismatch.actual_available_qty
          ),
          jsonb_build_object(
            'reservedQty',
            v_mismatch.reservation_difference,
            'availableQty',
            v_mismatch.actual_available_qty
              - v_mismatch.expected_available_qty
          ),
      ...
      'MARKETPLACE_ALLOCATION_CONSISTENCY' then
      ...
      loop
        ...
        values (
          ...
          jsonb_build_object(
            ...
            v_mismatch.order_id,
            'transactionId',
            v_mismatch.transaction_id
          ),
          null,
          null,
          'MARKETPLACE_EVENT',
          v_mismatch.external_event_ref,
          jsonb_build_object(
            'eventLineQuantity',
            v_mismatch.event_line_quantity,
            'eventLineCount',
            v_mismatch.event_line_count
          ),
          jsonb_build_object(
            'allocationQuantity',
            v_mismatch.allocation_quantity,
            'ledgerOutboundQuantity',
            v_mismatch.ledger_outbound_quantity,
            'allocationCount',
            v_mismatch.allocation_count,
            'ledgerEntryCount',
            v_mismatch.ledger_entry_count,
            'invalidAllocationCount',
            v_mismatch.invalid_allocation_count,
            'invalidLedgerCount',
            v_mismatch.invalid_ledger_count,
            'orphanLedgerCount',
            v_mismatch.orphan_ledger_count
          ),
          jsonb_build_object(
            'allocationQuantity',
            v_mismatch.allocation_quantity
        ...
        values (
          ...
          v_run_check_id,
          1,
          'MARKETPLACE_ALLOCATION_MISMATCH',
          'MARKETPLACE_SHIPMENT',
          jsonb_build_object(
            'eventId',
            v_mismatch.event_id,
          ...
          jsonb_build_object(
            ...
            v_mismatch.external_event_ref,
            'externalOrderRef',
            v_mismatch.external_order_ref,
            'channelId',
            v_mismatch.channel_id,
            'transactionId',
            v_mismatch.transaction_id,
            'ledgerSeqTo',
            v_ledger_seq_to,
      ...
      'RETURN_RECEIPT_CONSISTENCY' then

      select count(*)
      into v_checked_count
      from operations.return_events return_event
      where return_event.organization_id =
          p_organization_id
        and return_event.event_type_code in (
          'EXPECTED',
          'RECEIPT',
          'LOST'
        );
      ...
      loop
        ...
        values (
          ...
          'OPEN',
          case
            when v_mismatch.issue_code =
              'RETURN_NONPHYSICAL_EVENT_HAS_STOCK_EFFECT'
              then 'CRITICAL'
            else 'HIGH'
          end,
          'RETURN',
          jsonb_build_object(
            ...
            v_mismatch.event_id,
            'returnId',
            v_mismatch.return_id,
            'receiptId',
            v_mismatch.receipt_id,
            'transactionId',
            v_mismatch.transaction_id,
            'eventTypeCode',
            v_mismatch.event_type_code
          ),
          null,
          null,
          'RETURN_EVENT',
          ...
          jsonb_build_object(
            'receiptQuantity',
            v_mismatch.receipt_quantity,
            'ledgerQuarantineQuantity',
            v_mismatch.ledger_quarantine_quantity,
            'receiptLineCount',
            v_mismatch.receipt_line_count,
            'ledgerEntryCount',
            v_mismatch.ledger_entry_count,
            'invalidReceiptLineCount',
            v_mismatch.invalid_receipt_line_count,
            'invalidLedgerCount',
            v_mismatch.invalid_ledger_count,
            'orphanLedgerCount',
            v_mismatch.orphan_ledger_count,
            'unexpectedTransactionCount',
            v_mismatch.unexpected_transaction_count
          ),
          jsonb_build_object(
            'receiptQuantity',
            v_mismatch.receipt_quantity
              ...
              - case
                  when exists (
                    ...
                    where receipt.organization_id =
                      ...
                      and receipt.id =
                          v_mismatch.receipt_id
                      and receipt.stock_effect_code =
                          'LEGACY_QUARANTINE_INBOUND'
                  )
                    then v_mismatch.event_line_quantity
                  else 0
                end
          ),
          v_run_id,
        ...
        values (
          ...
          v_run_check_id,
          1,
          'RETURN_RECEIPT_CONSISTENCY_MISMATCH',
          'RETURN',
          jsonb_build_object(
            'eventId',
            v_mismatch.event_id,
          ...
          jsonb_build_object(
            ...
            v_mismatch.external_event_ref,
            'externalReturnRef',
            v_mismatch.external_return_ref,
            'receiptRef',
            v_mismatch.receipt_ref,
            'transactionId',
            v_mismatch.transaction_id,
            'ledgerSeqTo',
            v_ledger_seq_to,
      ...
      'RETURN_INSPECTION_CONSISTENCY' then

      select count(*)
      into v_checked_count
      from operations.return_inspections inspection
      where inspection.organization_id =
          p_organization_id;
      ...
      for v_mismatch in
        ...
        order by mismatch.inspection_id
      loop
        v_fingerprint := encode(
          extensions.digest(
            convert_to(
        ...
        values (
            ...
            case
              when exists (
                ...
                where inspection.organization_id =
                  ...
                  and inspection.id =
                      v_mismatch.inspection_id
                  and inspection.stock_effect_code =
          ...
          jsonb_build_object(
            ...
            v_mismatch.allocation_quantity,
            'sourceQuarantineQuantity',
            v_mismatch.source_quarantine_quantity,
            'destinationQuantity',
            v_mismatch.destination_quantity,
            'netQuantity',
            v_mismatch.net_quantity,
            'eventLineCount',
            v_mismatch.event_line_count,
            'allocationCount',
            v_mismatch.allocation_count,
            'sourceLedgerCount',
            v_mismatch.source_ledger_count,
            'destinationLedgerCount',
            v_mismatch.destination_ledger_count,
            'invalidHeaderCount',
            v_mismatch.invalid_header_count,
            'lineTotalMismatchCount',
            v_mismatch.line_total_mismatch_count,
            'invalidAllocationCount',
            v_mismatch.invalid_allocation_count,
            'invalidSourceLedgerCount',
            v_mismatch.invalid_source_ledger_count,
            'invalidDestinationLedgerCount',
            v_mismatch.invalid_destination_ledger_count,
            'orphanLedgerCount',
            v_mismatch.orphan_ledger_count
          ),
          jsonb_build_object(
        ...
        values (
          ...
          v_run_check_id,
          1,
          'RETURN_INSPECTION_CONSISTENCY_MISMATCH',
          'RETURN',
          jsonb_build_object(
            'inspectionId',
            v_mismatch.inspection_id,
      ...
      'DUPLICATE_SOURCE_EFFECT' then

      select count(*)
      into v_checked_count
      from (
        select
          'STOCK_TRANSACTION'::text as entity_type,
      ...
      for v_mismatch in
        ...
        order by
          mismatch.detection_scope,
          mismatch.violation_key
      loop
        v_fingerprint := encode(
          extensions.digest(
            convert_to(
              p_organization_id::text
                ...
                || v_rule_version
                || '|'
                || v_mismatch.detection_scope
                || '|'
                || v_mismatch.violation_key,
              'UTF8'
            ),
            'sha256'
          ),
        ...
        values (
          ...
          'OPEN',
          case
            when v_mismatch.transaction_count > 1
              or v_mismatch.absolute_quantity > 0
            then 'CRITICAL'
            else 'HIGH'
          end,
          case
            when v_mismatch.detection_scope =
              'SOURCE_TRANSACTION'
            then 'STOCK_SOURCE'
            else 'IDEMPOTENCY_COMMAND'
          end,
          jsonb_build_object(
            'detectionScope',
            v_mismatch.detection_scope,
            ...
            v_mismatch.violation_key,
            'transactionType',
            v_mismatch.transaction_type_code,
            'sourceType',
            v_mismatch.source_type_code,
            'sourceRef',
            v_mismatch.source_ref,
            'idempotencyCommandId',
            v_mismatch.idempotency_command_id
          ),
          null,
          null,
          case
            when v_mismatch.detection_scope =
              'SOURCE_TRANSACTION'
            then v_mismatch.source_type_code
            else 'IDEMPOTENCY_COMMAND'
          end,
          case
            when v_mismatch.detection_scope =
              'SOURCE_TRANSACTION'
            then v_mismatch.source_ref
            else v_mismatch.idempotency_command_id::text
          end,
          case
            when v_mismatch.detection_scope =
              'SOURCE_TRANSACTION'
          ...
          jsonb_build_object(
            ...
            v_mismatch.transaction_count,
            'commandCount',
            v_mismatch.command_count,
            'domainEffectCount',
            v_mismatch.domain_effect_count,
            'ledgerEntryCount',
            v_mismatch.ledger_entry_count,
            'absoluteQuantity',
            v_mismatch.absolute_quantity,
            'transactionIds',
            v_mismatch.transaction_ids,
            'commandIds',
            v_mismatch.command_ids,
            'domainEffects',
            v_mismatch.domain_effects
          ),
          case
            when v_mismatch.detection_scope =
              'SOURCE_TRANSACTION'
            then jsonb_build_object(
              'transactionCount',
              v_mismatch.transaction_count - 1,
              'commandCount',
              v_mismatch.command_count - 1,
              'domainEffectCount',
              v_mismatch.domain_effect_count
            )
            else jsonb_build_object(
              ...
              v_mismatch.command_count - 1,
              'domainEffectCount',
              v_mismatch.domain_effect_count - 1
            )
          end,
          v_run_id,
          v_run_id,
      ...
      'IMPOSSIBLE_PROJECTION_STATE' then
      ...
      from (
        select
          ...
          entry.product_id::text
            ...
            || entry.batch_id::text
            || ':'
            || entry.bucket_code as entity_key
        from inventory.stock_ledger_entries entry
        where entry.organization_id = p_organization_id
          and entry.ledger_seq <= v_ledger_seq_to
        group by
      ...
      loop
        ...
        values (
          ...
          jsonb_strip_nulls(
            jsonb_build_object(
              'quantity',
              v_mismatch.expected_quantity,
              'lastLedgerSeq',
              v_mismatch.expected_last_ledger_seq
            )
          ),
          jsonb_strip_nulls(
            jsonb_build_object(
              ...
              'lastLedgerSeq',
              v_mismatch.actual_last_ledger_seq,
              'projectionVersion',
              v_mismatch.projection_version
            )
          ),
          jsonb_strip_nulls(
            jsonb_build_object(
              'quantityDelta',
              case
                when v_mismatch.expected_quantity is not null
                  and v_mismatch.actual_quantity is not null
                then
                  v_mismatch.actual_quantity
                    - v_mismatch.expected_quantity
                else null
              ...
              case
                when
                  v_mismatch.expected_last_ledger_seq is not null
                  and v_mismatch.actual_last_ledger_seq is not null
                then
                  v_mismatch.actual_last_ledger_seq
                    - v_mismatch.expected_last_ledger_seq
                else null
...
set
  status_code = 'RESOLVED',
  resolved_at = clock_timestamp(),
  resolution_code = 'RULE_REPLACED_PHASE2',
  resolution_note =
    'Legacy return reconciliation rule was replaced by Phase 2 semantics.',
  resolved_by_user_id = null,
  resolved_by_process_name =
    'migration.202607180004_return_phase2_reconciliation_runtime',
  updated_at = clock_timestamp()
