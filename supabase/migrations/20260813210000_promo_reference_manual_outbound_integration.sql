...
-- 1. Redefine inventory.preview_manual_outbound_core dengan validasi promo reference
...
declare
  ...
  -- Variabel resolusi promo reference
  v_promo_ref_id uuid;
  v_promo_ref_code text;
  v_promo_ref_name text;
  v_promo_ref_active boolean;
  v_promo_ref_version bigint;
begin
  ...
  -- Validasi Promo Reference di tingkat database (Phase 2)
  if v_reason_code = 'PROMO' and v_reference is not null then
    if p_lock_basis then
      select
        promo.id,
        promo.code,
      ...
      where promo.organization_id = p_organization_id
        and catalog.normalize_master_identifier(promo.code) = catalog.normalize_master_identifier(v_reference)
      for update;
    else
      select
        promo.id,
    ...
    if not found then
      v_blockers := v_blockers || jsonb_build_array(
        jsonb_build_object(
          'code', 'OUTBOUND_PROMO_REFERENCE_INVALID',
          'scope', 'REQUEST',
          'message', 'Referensi promo tidak ditemukan.'
        )
      );
    elsif not v_promo_ref_active then
      v_blockers := v_blockers || jsonb_build_array(
        jsonb_build_object(
          'code', 'OUTBOUND_PROMO_REFERENCE_INVALID',
          'scope', 'REQUEST',
          'message', 'Referensi promo tidak aktif.'
        )
      );
    end if;
  end if;
  ...
  v_authoritative_basis := jsonb_build_object(
    ...
    'promoReference', case
      when v_reason_code = 'PROMO' and v_promo_ref_id is not null then
        jsonb_build_object(
          'id', v_promo_ref_id,
          'code', v_promo_ref_code,
          'name', v_promo_ref_name,
          'isActive', v_promo_ref_active,
          'rowVersion', v_promo_ref_version
        )
      else null
    end,
    'schemaVersion', 1
...
-- 2. Redefine api.post_manual_outbound (signature 8 parameter) untuk persistence enrichment
...
begin
  ...
  -- Hitung request hash menggunakan p_metadata orisinal
  v_request_hash := encode(
    extensions.digest(
      convert_to(
        jsonb_build_object(
  ...
  -- Resolusi Promo reference di tingkat database (Phase 2)
  -- Dilakukan SETELAH check replay idempotensi agar replay aman dari perubahan status master data promo di kemudian hari
  if v_reason_code = 'PROMO' then
    v_reference := nullif(btrim(coalesce(v_metadata ->> 'reference', '')), '');
    ...
    if not found then
      raise exception using
        errcode = 'P0001',
        message = 'OUTBOUND_PROMO_REFERENCE_INVALID';
    end if;

    -- Perkaya v_metadata dengan snapshot DB
    v_metadata := v_metadata || jsonb_build_object(
      'promoReference', jsonb_build_object(
        'id', v_promo_ref_id,
        'code', v_promo_ref_code,
        'name', v_promo_ref_name,
        'rowVersion', v_promo_ref_version
