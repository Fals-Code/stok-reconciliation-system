...
create or replace function api.product_stock_explanation(
  p_product_id uuid
)
returns jsonb
language sql
...
as $$
  ...
  grouped_evidence as materialized (
    select coalesce(
      jsonb_agg(
        ...
        order by grouped.transaction_type_code,
          grouped.reason_code_snapshot,
          grouped.channel_code_snapshot,
          grouped.source_type_code
      ),
      '[]'::jsonb
    ) as movements
    from (
  ...
  select jsonb_build_object(
    ...
    'ledger', jsonb_build_object(
      'sellableQty', ledger_totals.sellable_qty,
      'quarantineQty', ledger_totals.quarantine_qty,
      'damagedQty', ledger_totals.damaged_qty,
      'onHandQty', ledger_totals.sellable_qty + ledger_totals.quarantine_qty + ledger_totals.damaged_qty
    ),
    'projection', jsonb_build_object(
      'sellableQty', projection.sellable_qty,
      'quarantineQty', projection.quarantine_qty,
      'damagedQty', projection.damaged_qty,
      'reservedQty', projection.reserved_qty,
      'availableQty', projection.sellable_qty - projection.reserved_qty,
      'onHandQty', projection.sellable_qty + projection.quarantine_qty + projection.damaged_qty
    ),
    'comparison', jsonb_build_object(
      'sellableMatches', ledger_totals.sellable_qty = projection.sellable_qty,
      'quarantineMatches', ledger_totals.quarantine_qty = projection.quarantine_qty,
      'damagedMatches', ledger_totals.damaged_qty = projection.damaged_qty,
      'onHandMatches', ledger_totals.sellable_qty + ledger_totals.quarantine_qty + ledger_totals.damaged_qty = projection.sellable_qty + projection.quarantine_qty + projection.damaged_qty
    ),
  ...
  cross join grouped_evidence
$$;

revoke all on function api.product_stock_explanation(uuid)
from public, anon;

grant execute on function api.product_stock_explanation(uuid)
