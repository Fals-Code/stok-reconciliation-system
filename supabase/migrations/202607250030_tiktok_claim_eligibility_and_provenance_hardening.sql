...
begin
  if p_organization_id is null
    ...
    raise exception using
      errcode = 'P0001',
      message = 'RETURN_CLAIM_REQUEST_INVALID';
  end if;
  ...
  if p_claim_type_code not in (
    ...
    raise exception using
      errcode = 'P0001',
      message = 'RETURN_CLAIM_TYPE_INVALID';
  end if;
  ...
  if not found then
    raise exception using
      errcode = 'P0001',
      message = 'TIKTOK_RETURN_NOT_FOUND';
  end if;
  ...
  if v_item_count <> v_distinct_count then
    raise exception using
      errcode = 'P0001',
      message = 'RETURN_CLAIM_DUPLICATE_ITEM';
  end if;
  ...
  if exists (
    ...
    raise exception using
      errcode = 'P0001',
      message = 'RETURN_CLAIM_ITEM_INVALID';
  end if;
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(
      p_organization_id::text || ':CREATE_TIKTOK_RETURN_CLAIM:' || v_key,
      0
    )
  );
  ...
  /* Lock every requested item in UUID order before reading claim capacity. */
  perform 1
  from jsonb_array_elements(p_items) requested(value)
  ...
  /* The return-item locks serialize all claims for each canonical item. */
  for v_item in
    select
      return_item.*,
      (requested.value->>'quantity')::bigint requested_qty
    from jsonb_array_elements(p_items) requested(value)
    join operations.return_items return_item
      on return_item.organization_id = p_organization_id
     and return_item.return_id = p_return_id
    ...
    order by return_item.id
  loop
    v_net_lost_qty := greatest(
      v_item.lost_qty - coalesce(v_item.late_arrival_qty, 0),
      0
    );
  ...
  values (
    ...
    40,
    'Asia/Jakarta',
    'INTERNAL_RETURN_CREATED_AT',
    v_return.created_at + interval '40 days',
    'TIKTOK_RETURN_CREATED_AT_V1',
    v_actor,
    v_command_id,
    v_hash
  );
  ...
  loop
    insert into operations.return_claim_items(
      ...
      return_item_id,
      quantity,
      eligible_lost_qty_snapshot,
      product_id,
      product_sku_snapshot,
      source_line_ref_snapshot,
      canonical_components_snapshot
    )
    select
      p_organization_id,
      v_claim,
      ...
      jsonb_build_array(
        jsonb_build_object(
          ...
          'provenanceKind', case
            when provenance.source_line_id is null then 'RETURN_ITEM'
            when provenance.bundle_recipe_id is null then 'SINGLE_PRODUCT_SOURCE'
          ...
          'marketplaceSourceLineId', provenance.source_line_id,
          'marketplaceSourceLineRef', provenance.source_line_ref,
          'listingId', provenance.listing_id,
          'mappingVersion', provenance.mapping_version,
          'mappingFingerprint', provenance.mapping_fingerprint,
          'singleListingVersionId', provenance.single_listing_version_id,
          'bundleRecipeId', provenance.bundle_recipe_id,
          'recipeComponentId', provenance.recipe_component_id,
          'componentNo', provenance.component_no,
          'unitQuantityPerListing', provenance.unit_quantity_per_listing,
          'listingQuantity', provenance.listing_quantity,
          'expandedQuantity', provenance.expanded_quantity
        )
      )
    from operations.marketplace_order_items order_item
    left join lateral (
      select
        source_line.id source_line_id,
      ...
      where source_line.organization_id = p_organization_id
        ...
        and source_line.source_line_ref = v_item.source_line_ref
      order by component.component_no nulls last, component.id nulls last
      limit 1
    ) provenance on true
    ...
    if not found then
      ...
      values (
        ...
        jsonb_build_array(
          jsonb_build_object(
            'snapshotSchemaVersion', 2,
            'provenanceKind', 'RETURN_ITEM',
            'returnItemId', v_item.id,
            'productId', v_item.product_id,
            'productSku', v_item.product_sku_snapshot,
            'sourceLineRef', v_item.source_line_ref,
            'marketplaceSourceLineId', null,
            'marketplaceSourceLineRef', null,
            'listingId', null,
            'mappingVersion', null,
