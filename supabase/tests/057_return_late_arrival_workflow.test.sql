...
create temp table late_auth_fixture(admin_user_id uuid primary key) on commit drop;
insert into auth.users(instance_id,id,aud,role,email,email_confirmed_at,raw_app_meta_data,raw_user_meta_data,created_at,updated_at,is_sso_user,is_anonymous) values
  ('00000000-0000-0000-0000-000000000000','95700000-0000-4000-8000-000000000057','authenticated','authenticated','pgtap.late.057.admin@glowlab.invalid','2026-07-01 00:00:00+07','{"provider":"email","providers":["email"]}'::jsonb,'{}'::jsonb,'2026-07-01 00:00:00+07','2026-07-01 00:00:00+07',false,false);
insert into app.user_profiles(user_id,organization_id,display_name,employee_code,role_code,is_active) values
  ('95700000-0000-4000-8000-000000000057','00000000-0000-4000-8000-000000000001','pgTAP Late 057 Admin','PGTAP-LATE-057','ADMIN',true);
insert into late_auth_fixture values ('95700000-0000-4000-8000-000000000057');
...
select set_config('request.jwt.claim.sub',(select admin_user_id::text from late_auth_fixture),true);
select set_config('request.jwt.claim.role','authenticated',true);
select set_config('request.jwt.claims',jsonb_build_object('sub',(select admin_user_id::text from late_auth_fixture),'role','authenticated')::text,true);
set local role authenticated;

insert into late_results select 'SPLIT_BATCH',api.create_product_batch('00000000-0000-4000-8000-000000000001','057-SPLIT-BATCH','30000000-0000-4000-8000-000000000002','CLN-2610-SPLIT','2026-10-31',null,null,'STANDARD','split-batch provenance fixture');
insert into late_results select 'SPLIT_RECEIPT',api.post_receipt('00000000-0000-4000-8000-000000000001','057-SPLIT-RECEIPT','057-SPLIT-RECEIPT','2026-07-01 08:00:00+07',jsonb_build_array(jsonb_build_object('productId','30000000-0000-4000-8000-000000000002','batchId',(select result->>'batchId' from late_results where kind='SPLIT_BATCH'),'quantity',2,'sourceLineRef','057-SPLIT-INBOUND')),'split-batch provenance fixture','{}');
...
create temp table late_fixture as
select returned.id return_id,item_a.id product_a_return_item_id,item_b.id product_b_return_item_id,
  (select allocation.id from operations.marketplace_ship_allocations allocation join operations.marketplace_event_lines line on line.organization_id=allocation.organization_id and line.id=allocation.event_line_id join operations.marketplace_events event on event.organization_id=line.organization_id and event.id=line.event_id where allocation.organization_id=returned.organization_id and event.external_event_ref='057-SHIP-EVENT' and line.source_line_ref='057-A' and allocation.product_id=item_a.product_id and allocation.quantity_allocated=3) product_a_allocation_id
 ,(select allocation.id from operations.marketplace_ship_allocations allocation join operations.marketplace_event_lines line on line.organization_id=allocation.organization_id and line.id=allocation.event_line_id join operations.marketplace_events event on event.organization_id=line.organization_id and event.id=line.event_id where allocation.organization_id=returned.organization_id and event.external_event_ref='057-SHIP-EVENT' and line.source_line_ref='057-B' and allocation.product_id=item_b.product_id and allocation.allocation_no=1) product_b_allocation_one_id
 ,(select allocation.id from operations.marketplace_ship_allocations allocation join operations.marketplace_event_lines line on line.organization_id=allocation.organization_id and line.id=allocation.event_line_id join operations.marketplace_events event on event.organization_id=line.organization_id and event.id=allocation.event_id where allocation.organization_id=returned.organization_id and event.external_event_ref='057-SHIP-EVENT' and line.source_line_ref='057-B' and allocation.product_id=item_b.product_id and allocation.allocation_no=2) product_b_allocation_two_id
from operations.returns returned join operations.return_items item_a on item_a.organization_id=returned.organization_id and item_a.return_id=returned.id and item_a.product_id='30000000-0000-4000-8000-000000000001'::uuid and item_a.source_line_ref='057-A' join operations.return_items item_b on item_b.organization_id=returned.organization_id and item_b.return_id=returned.id and item_b.product_id='30000000-0000-4000-8000-000000000002'::uuid and item_b.source_line_ref='057-B' where returned.organization_id='00000000-0000-4000-8000-000000000001'::uuid and returned.external_return_ref='057-RETURN';
...
insert into late_results select 'LATE',api.confirm_late_return_arrival('00000000-0000-4000-8000-000000000001','057-LATE','057-RETURN','057-LATE-REF','057-LATE-RECEIPT','2026-07-06 12:00:00+07',jsonb_build_array(jsonb_build_object('returnItemId',(select product_a_return_item_id::text from late_fixture),'quantity',1,'marketplaceShipAllocationId',(select product_a_allocation_id::text from late_fixture)),jsonb_build_object('returnItemId',(select product_b_return_item_id::text from late_fixture),'quantity',1,'marketplaceShipAllocationId',(select product_b_allocation_one_id::text from late_fixture)),jsonb_build_object('returnItemId',(select product_b_return_item_id::text from late_fixture),'quantity',1,'marketplaceShipAllocationId',(select product_b_allocation_two_id::text from late_fixture)),jsonb_build_object('returnItemId',(select product_b_return_item_id::text from late_fixture),'quantity',1,'marketplaceShipAllocationId',null)),'fixture','{}');
...
select is((select coalesce(jsonb_agg(jsonb_build_object('returnItemId',grouped.return_item_id,'lostEventLineId',grouped.lost_event_line_id,'quantity',grouped.quantity) order by grouped.return_item_id,grouped.occurred_at,grouped.event_id,grouped.line_no,grouped.lost_event_line_id),'[]'::jsonb) from (select allocation.return_item_id,allocation.lost_event_line_id,sum(allocation.quantity)::bigint quantity,lost_event.occurred_at,lost_event.id event_id,lost_line.line_no from operations.return_late_arrival_lines allocation join operations.return_event_lines lost_line on lost_line.organization_id=allocation.organization_id and lost_line.id=allocation.lost_event_line_id join operations.return_events lost_event on lost_event.organization_id=lost_line.organization_id and lost_event.id=lost_line.event_id where allocation.late_arrival_id=(select (result->>'lateArrivalId')::uuid from late_results where kind='LATE') group by allocation.return_item_id,allocation.lost_event_line_id,lost_event.occurred_at,lost_event.id,lost_line.line_no) grouped),(select jsonb_build_array(jsonb_build_object('returnItemId',(select product_a_return_item_id from late_fixture),'lostEventLineId',(select id from operations.return_event_lines where source_line_ref='057-LOST-A-ONE' and return_item_id=(select product_a_return_item_id from late_fixture)),'quantity',1),jsonb_build_object('returnItemId',(select product_b_return_item_id from late_fixture),'lostEventLineId',(select id from operations.return_event_lines where source_line_ref='057-LOST-B-ONE' and return_item_id=(select product_b_return_item_id from late_fixture)),'quantity',2),jsonb_build_object('returnItemId',(select product_b_return_item_id from late_fixture),'lostEventLineId',(select id from operations.return_event_lines where source_line_ref='057-LOST-B-TWO' and return_item_id=(select product_b_return_item_id from late_fixture)),'quantity',1))),'late arrival stores deterministic allocations across lost lines');
...
select set_config('request.jwt.claim.sub',(select admin_user_id::text from late_auth_fixture),true); select set_config('request.jwt.claim.role','authenticated',true); select set_config('request.jwt.claims',jsonb_build_object('sub',(select admin_user_id::text from late_auth_fixture),'role','authenticated')::text,true); set local role authenticated;
insert into late_results select 'INSPECT',api.inspect_return('00000000-0000-4000-8000-000000000001','057-INSPECT','057-RETURN','057-INSPECT-REF','2026-07-06 14:00:00+07',jsonb_build_array(jsonb_build_object('receiptLineId',(select receipt_line_id::text from api.return_receipt_lines where receipt_id=(select id from operations.return_receipts where organization_id='00000000-0000-4000-8000-000000000001'::uuid and receipt_ref='057-LATE-RECEIPT') and return_item_id=(select product_a_return_item_id from late_fixture) and quantity_received=1),'sellableQuantity',1,'damagedQuantity',0,'sourceLineRef','057-INSPECT-A'),jsonb_build_object('receiptLineId',(select receipt_line_id::text from api.return_receipt_lines where receipt_id=(select id from operations.return_receipts where organization_id='00000000-0000-4000-8000-000000000001'::uuid and receipt_ref='057-LATE-RECEIPT') and marketplace_ship_allocation_id=(select product_b_allocation_one_id from late_fixture)),'sellableQuantity',0,'damagedQuantity',1,'sourceLineRef','057-INSPECT-B-ONE'),jsonb_build_object('receiptLineId',(select receipt_line_id::text from api.return_receipt_lines where receipt_id=(select id from operations.return_receipts where organization_id='00000000-0000-4000-8000-000000000001'::uuid and receipt_ref='057-LATE-RECEIPT') and marketplace_ship_allocation_id=(select product_b_allocation_two_id from late_fixture)),'sellableQuantity',0,'damagedQuantity',1,'sourceLineRef','057-INSPECT-B-TWO'),jsonb_build_object('receiptLineId',(select receipt_line_id::text from api.return_receipt_lines where receipt_id=(select id from operations.return_receipts where organization_id='00000000-0000-4000-8000-000000000001'::uuid and receipt_ref='057-LATE-RECEIPT') and return_item_id=(select product_b_return_item_id from late_fixture) and marketplace_ship_allocation_id is null),'sellableQuantity',0,'damagedQuantity',1,'sourceLineRef','057-INSPECT-B-UNVERIFIED')),'fixture','{}');
...
select is((select count(*) from operations.return_late_arrival_lines allocation join operations.return_receipt_lines receipt_line on receipt_line.organization_id=allocation.organization_id and receipt_line.late_arrival_line_id=allocation.id where allocation.late_arrival_id=(select (result->>'lateArrivalId')::uuid from late_results where kind='LATE') and receipt_line.stock_effect_code='NONE'),4::bigint,'each late-arrival allocation has one typed stock-neutral receipt line');
select is((select count(*) from operations.return_receipt_lines receipt_line where receipt_line.receipt_id=(select (result->>'receiptId')::uuid from late_results where kind='LATE') and receipt_line.return_item_id=(select product_b_return_item_id from late_fixture) and receipt_line.marketplace_ship_allocation_id is not null),2::bigint,'split shipment creates two exact-provenance receipt lines for the same return item');
select is((select jsonb_agg(jsonb_build_object('allocationId',receipt_line.marketplace_ship_allocation_id,'sourceBatchId',receipt_line.source_batch_id,'quantity',receipt_line.quantity_received) order by receipt_line.marketplace_ship_allocation_id) from operations.return_receipt_lines receipt_line where receipt_line.receipt_id=(select (result->>'receiptId')::uuid from late_results where kind='LATE') and receipt_line.return_item_id=(select product_b_return_item_id from late_fixture) and receipt_line.marketplace_ship_allocation_id is not null),(select jsonb_agg(jsonb_build_object('allocationId',allocation.id,'sourceBatchId',allocation.batch_id,'quantity',1) order by allocation.id) from operations.marketplace_ship_allocations allocation where allocation.id in ((select product_b_allocation_one_id from late_fixture),(select product_b_allocation_two_id from late_fixture))),'split receipt provenance preserves each allocation and source batch exactly');
select is((select count(*) from operations.return_late_arrival_lines line join operations.return_receipt_lines receipt_line on receipt_line.organization_id=line.organization_id and receipt_line.late_arrival_line_id=line.id where line.late_arrival_id=(select (result->>'lateArrivalId')::uuid from late_results where kind='LATE') and line.return_item_id=(select product_b_return_item_id from late_fixture) and receipt_line.marketplace_ship_allocation_id is not null),2::bigint,'split shipment creates two exact-provenance late-arrival lines for the same return item');
select is((select count(*) from operations.return_receipt_lines receipt_line where receipt_line.receipt_id=(select (result->>'receiptId')::uuid from late_results where kind='LATE_REPLAY')),4::bigint,'split shipment exact replay does not create additional receipt lines');
