...
insert into claim_notification_results select 'RESERVE',api.apply_marketplace_event('00000000-0000-4000-8000-000000000001','056-TT-RESERVE','TIKTOK_SHOP','RESERVE','056-TT-RESERVE-EVENT','056-TT-ORDER','2026-06-01 09:00:00+07',jsonb_build_array(jsonb_build_object('productId','30000000-0000-4000-8000-000000000001','quantity',1,'sourceLineRef','056-TT-LINE')),'fixture','{}');
insert into claim_notification_results select 'SHIP',api.apply_marketplace_event('00000000-0000-4000-8000-000000000001','056-TT-SHIP','TIKTOK_SHOP','SHIP','056-TT-SHIP-EVENT','056-TT-ORDER','2026-06-02 09:00:00+07',jsonb_build_array(jsonb_build_object('productId','30000000-0000-4000-8000-000000000001','quantity',1,'sourceLineRef','056-TT-LINE')),'fixture','{}');
insert into claim_notification_results select 'EXPECTED',api.create_expected_return('00000000-0000-4000-8000-000000000001','056-TT-EXPECTED','TIKTOK_SHOP','056-TT-RETURN','056-TT-ORDER','2026-06-03 10:00:00+07',jsonb_build_array(jsonb_build_object('productId','30000000-0000-4000-8000-000000000001','quantity',1,'sourceLineRef','056-TT-LINE')),'RETURN_REQUESTED','fixture','{}');
...
insert into claim_notification_results select 'LOST',api.mark_return_lost('00000000-0000-4000-8000-000000000001','056-TT-LOST','056-TT-RETURN','056-TT-LOST-EVENT','2026-06-04 11:00:00+07',jsonb_build_array(jsonb_build_object('returnItemId',(select item.id::text from operations.return_items item join operations.returns header on header.id=item.return_id where header.external_return_ref='056-TT-RETURN'),'quantity',1,'sourceLineRef','056-TT-LOST-LINE')),'fixture','{}');
insert into claim_notification_results select 'CLAIM',api.create_tiktok_return_claim('00000000-0000-4000-8000-000000000001','056-TT-CLAIM',(select id from operations.returns where external_return_ref='056-TT-RETURN'),'LOST_RETURN',jsonb_build_array(jsonb_build_object('returnItemId',(select item.id::text from operations.return_items item join operations.returns header on header.id=item.return_id where header.external_return_ref='056-TT-RETURN'),'quantity',1)),'2026-06-20 10:00:00+07');
reset role;
select is((select count(*) from notification.outbox_events where organization_id='00000000-0000-4000-8000-000000000001'::uuid and entity_type_code='RETURN_CLAIM' and entity_id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM') and event_type_code='TIKTOK_CLAIM_DEADLINE_EVALUATION_REQUEST'),1::bigint,'trusted claim creation writes one claim-linked deadline-evaluation outbox request');
select ok(exists(select 1 from notification.outbox_events where organization_id='00000000-0000-4000-8000-000000000001'::uuid and entity_type_code='RETURN_CLAIM' and entity_id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM') and payload->>'claimId'=(select result->>'claimId' from claim_notification_results where kind='CLAIM')),'claim outbox payload remains traceable to the claim');
select is((select count(*) from notification.outbox_events where organization_id='00000000-0000-4000-8000-000000000001'::uuid and entity_type_code='RETURN_CLAIM' and entity_id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM') and event_type_code='TIKTOK_CLAIM_DEADLINE_EVALUATION_REQUEST'),1::bigint,'claim creation effect has one durable outbox record before evaluator execution');
select set_config('request.jwt.claim.sub','720d8ca5-3f95-4a20-8063-d24981ad551d',true);
select set_config('request.jwt.claim.role','authenticated',true);
select set_config('request.jwt.claims','{"sub":"720d8ca5-3f95-4a20-8063-d24981ad551d","role":"authenticated"}',true);
set local role authenticated;
insert into claim_notification_results select 'CLAIM_REPLAY',api.create_tiktok_return_claim('00000000-0000-4000-8000-000000000001','056-TT-CLAIM',(select (result->>'returnId')::uuid from claim_notification_results where kind='RETURN'),'LOST_RETURN',jsonb_build_array(jsonb_build_object('returnItemId',(select result->>'returnItemId' from claim_notification_results where kind='RETURN_ITEM'),'quantity',1)),'2026-06-20 10:00:00+07');
reset role;
...
insert into claim_notification_results select 'D14',notification.evaluate_tiktok_claim_deadlines('00000000-0000-4000-8000-000000000001','056-d14',(select deadline_at-interval '14 days' from operations.return_claims where id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM')),'pgtap.056');
...
values ('00000000-0000-0000-0000-000000000000','95600000-0000-4000-8000-000000000001','authenticated','authenticated','pgtap.claim.notification.second-admin@glowlab.invalid','2026-06-01 00:00:00+07','{"provider":"email","providers":["email"]}'::jsonb,'{}'::jsonb,'2026-06-01 00:00:00+07','2026-06-01 00:00:00+07',false,false);
insert into app.user_profiles(user_id,organization_id,display_name,employee_code,role_code,is_active)
values ('95600000-0000-4000-8000-000000000001','00000000-0000-4000-8000-000000000001','pgTAP Claim Notification Second Admin','PGTAP-CLM-NOTIF-2','ADMIN',true);
...
insert into claim_notification_results select 'D7',notification.evaluate_tiktok_claim_deadlines('00000000-0000-4000-8000-000000000001','056-d7',(select deadline_at-interval '7 days' from operations.return_claims where id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM')),'pgtap.056');
...
insert into claim_notification_results select 'D3',notification.evaluate_tiktok_claim_deadlines('00000000-0000-4000-8000-000000000001','056-d3',(select deadline_at-interval '3 days' from operations.return_claims where id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM')),'pgtap.056');
...
insert into claim_notification_results select 'D1',notification.evaluate_tiktok_claim_deadlines('00000000-0000-4000-8000-000000000001','056-d1',(select deadline_at-interval '1 day' from operations.return_claims where id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM')),'pgtap.056');
...
insert into claim_notification_results select 'DUE',notification.evaluate_tiktok_claim_deadlines('00000000-0000-4000-8000-000000000001','056-due',(select deadline_at from operations.return_claims where id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM')),'pgtap.056');
...
select ok(exists(select 1 from notification.rule_runs where organization_id='00000000-0000-4000-8000-000000000001'::uuid and rule_code_snapshot='CLAIM_DEADLINE' and idempotency_key='056-overdue' and status_code='SUCCEEDED'),'evaluator records successful rule run');
select is((select count(*) from notification.rule_runs where organization_id='00000000-0000-4000-8000-000000000001'::uuid and rule_code_snapshot='CLAIM_DEADLINE' and idempotency_key='056-overdue'),1::bigint,'one durable rule run is stored for an evaluator run key');
...
select is((select count(*) from notification.rule_runs where organization_id='00000000-0000-4000-8000-000000000001'::uuid and rule_code_snapshot='CLAIM_DEADLINE' and idempotency_key='056-overdue'),1::bigint,'rule-run replay does not create a second run');
...
values (gen_random_uuid(),'00000000-0000-4000-8000-000000000001','LEGACY_IMPORTED_TIKTOK_RETURN_CLAIM','056-basis-missing-fixture',repeat('0',64),'SUCCEEDED',clock_timestamp(),'{}');
with claim as (
...
insert into claim_notification_results select 'BASIS_MISSING_INITIAL',notification.evaluate_tiktok_claim_deadlines('00000000-0000-4000-8000-000000000001','056-basis-missing-initial','2026-07-01 10:00:00+07','pgtap.056');
...
insert into claim_notification_results select 'BASIS_MISSING_REPLAY',notification.evaluate_tiktok_claim_deadlines('00000000-0000-4000-8000-000000000001','056-basis-missing-initial','2026-07-01 10:00:00+07','pgtap.056');
...
select set_config('request.jwt.claims','{"sub":"720d8ca5-3f95-4a20-8063-d24981ad551d","role":"authenticated"}',true);
set local role authenticated;
insert into claim_notification_results select 'BASIS_MISSING_CANCEL',api.cancel_tiktok_return_claim('00000000-0000-4000-8000-000000000001','056-basis-missing-cancel',(select (result->>'claimId')::uuid from claim_notification_results where kind='BASIS_FIXTURE'),'legacy fixture terminal resolution','2026-07-02 10:00:00+07');
reset role;
insert into claim_notification_results select 'BASIS_MISSING_RESOLUTION',notification.evaluate_tiktok_claim_deadlines('00000000-0000-4000-8000-000000000001','056-basis-missing-resolution','2026-07-02 10:00:00+07','pgtap.056');
...
select is((select count(*) from notification.notifications where id=(select id from basis_missing_episode) and organization_id<>'00000000-0000-4000-8000-000000000001'::uuid),0::bigint,'no foreign organization owns the basis-missing episode');
...
select lives_ok($$select notification.set_notification_read_state('00000000-0000-4000-8000-000000000001',(select id from evaluator_episode),'720d8ca5-3f95-4a20-8063-d24981ad551d','READ',(select deadline_at+interval '3 microseconds' from operations.return_claims where id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM')))$$,'Admin A can read an overdue notification');
select lives_ok($$select notification.set_notification_read_state('00000000-0000-4000-8000-000000000001',(select id from evaluator_episode),'95600000-0000-4000-8000-000000000001','READ',(select deadline_at+interval '3 microseconds' from operations.return_claims where id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM')))$$,'Admin B can independently read an overdue notification');
select lives_ok($$select notification.set_notification_read_state('00000000-0000-4000-8000-000000000001',(select id from evaluator_episode),'720d8ca5-3f95-4a20-8063-d24981ad551d','ARCHIVED_FOR_USER',(select deadline_at+interval '4 microseconds' from operations.return_claims where id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM')))$$,'Admin A can archive only their presentation state');
select is((select read_state_code from notification.user_states where notification_id=(select id from evaluator_episode) and user_id='720d8ca5-3f95-4a20-8063-d24981ad551d'),'ARCHIVED','Admin A archive is stored per user');
select is((select read_state_code from notification.user_states where notification_id=(select id from evaluator_episode) and user_id='95600000-0000-4000-8000-000000000001'),'READ','Admin B state is unaffected by Admin A archive');
select is((select count(*) from notification.user_states where notification_id=(select id from evaluator_episode) and user_id in ('720d8ca5-3f95-4a20-8063-d24981ad551d','95600000-0000-4000-8000-000000000001')),2::bigint,'two Admin actions do not duplicate user-state rows');
select lives_ok($$select notification.acknowledge_notification('00000000-0000-4000-8000-000000000001',(select id from evaluator_episode),(select deadline_at+interval '5 microseconds' from operations.return_claims where id=(select (result->>'claimId')::uuid from claim_notification_results where kind='CLAIM')),gen_random_uuid(),'720d8ca5-3f95-4a20-8063-d24981ad551d','critical overdue acknowledged')$$,'ACKNOWLEDGE follows the existing critical-note contract');
...
create temp table fresh_timeline as select coalesce(max(last_seen_at),'2026-07-01 00:00:00+07'::timestamptz) max_last_seen_at from notification.notifications where organization_id='00000000-0000-4000-8000-000000000001'::uuid;
insert into claim_notification_results select 'FRESH_RESERVE',api.apply_marketplace_event('00000000-0000-4000-8000-000000000001','056-FRESH-RESERVE','TIKTOK_SHOP','RESERVE','056-FRESH-RESERVE-EVENT','056-FRESH-ORDER',(select max_last_seen_at-interval '27 days' from fresh_timeline),jsonb_build_array(jsonb_build_object('productId','30000000-0000-4000-8000-000000000001','quantity',1,'sourceLineRef','056-FRESH-LINE')),'fixture','{}');
insert into claim_notification_results select 'FRESH_SHIP',api.apply_marketplace_event('00000000-0000-4000-8000-000000000001','056-FRESH-SHIP','TIKTOK_SHOP','SHIP','056-FRESH-SHIP-EVENT','056-FRESH-ORDER',(select max_last_seen_at-interval '26 days' from fresh_timeline),jsonb_build_array(jsonb_build_object('productId','30000000-0000-4000-8000-000000000001','quantity',1,'sourceLineRef','056-FRESH-LINE')),'fixture','{}');
insert into claim_notification_results select 'FRESH_EXPECTED',api.create_expected_return('00000000-0000-4000-8000-000000000001','056-FRESH-EXPECTED','TIKTOK_SHOP','056-FRESH-RETURN','056-FRESH-ORDER',(select max_last_seen_at-interval '25 days' from fresh_timeline),jsonb_build_array(jsonb_build_object('productId','30000000-0000-4000-8000-000000000001','quantity',1,'sourceLineRef','056-FRESH-LINE')),'RETURN_REQUESTED','fixture','{}');
insert into claim_notification_results select 'FRESH_LOST',api.mark_return_lost('00000000-0000-4000-8000-000000000001','056-FRESH-LOST','056-FRESH-RETURN','056-FRESH-LOST-EVENT',(select max_last_seen_at-interval '24 days' from fresh_timeline),jsonb_build_array(jsonb_build_object('returnItemId',(select id::text from operations.return_items where return_id=(select id from operations.returns where external_return_ref='056-FRESH-RETURN')),'quantity',1,'sourceLineRef','056-FRESH-LINE')),'fixture','{}');
insert into claim_notification_results select 'FRESH_CLAIM',api.create_tiktok_return_claim('00000000-0000-4000-8000-000000000001','056-FRESH-CLAIM',(select id from operations.returns where external_return_ref='056-FRESH-RETURN'),'LOST_RETURN',jsonb_build_array(jsonb_build_object('returnItemId',(select id::text from operations.return_items where return_id=(select id from operations.returns where external_return_ref='056-FRESH-RETURN')),'quantity',1)),'2026-06-20 10:00:00+07'); reset role;
