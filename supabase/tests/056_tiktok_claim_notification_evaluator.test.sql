...
select matches(pg_get_functiondef('notification.evaluate_tiktok_claim_deadlines(uuid,text,timestamp with time zone,text)'::regprocedure),'''D14'',''D7'',''D3'',''D1'',''DUE_TODAY'',''OVERDUE''','evaluator ranks deadline stages deterministically for direction');
select matches(pg_get_functiondef('notification.evaluate_tiktok_claim_deadlines(uuid,text,timestamp with time zone,text)'::regprocedure),'v_direction:=case','evaluator derives direction from the active notification stage rather than claim status');
...
grant select,insert,update on claim_notification_results to service_role;
...
grant select,update on evaluator_fixture to service_role;
...
select set_config('request.jwt.claims','{"sub":"720d8ca5-3f95-4a20-8063-d24981ad551d","role":"authenticated"}',true);
grant select, insert, update on claim_notification_results to service_role;
grant select on evaluator_fixture to service_role;
...
create temp table lifecycle_b_stock_before as select jsonb_build_object('transactions',(select coalesce(jsonb_agg(to_jsonb(transaction_row) order by transaction_row.id),'[]'::jsonb) from inventory.stock_transactions transaction_row where transaction_row.organization_id='00000000-0000-4000-8000-000000000056'::uuid),'ledgerEntries',(select coalesce(jsonb_agg(to_jsonb(ledger_row) order by ledger_row.id),'[]'::jsonb) from inventory.stock_ledger_entries ledger_row where ledger_row.organization_id='00000000-0000-4000-8000-000000000056'::uuid),'reservations',(select coalesce(jsonb_agg(to_jsonb(reservation) order by reservation.id),'[]'::jsonb) from inventory.stock_reservations reservation where reservation.organization_id='00000000-0000-4000-8000-000000000056'::uuid),'productPositions',(select coalesce(jsonb_agg(to_jsonb(position) order by position.product_id),'[]'::jsonb) from inventory.stock_product_positions position where position.organization_id='00000000-0000-4000-8000-000000000056'::uuid),'batchPositions',(select coalesce(jsonb_agg(to_jsonb(balance) order by balance.product_id,balance.batch_id),'[]'::jsonb) from inventory.stock_batch_balances balance where balance.organization_id='00000000-0000-4000-8000-000000000056'::uuid)) snapshot;
set local role service_role;
insert into lifecycle_fixture_results select 'LIFECYCLE_B_EXPIRE',api.evaluate_tiktok_return_claim_deadline('00000000-0000-4000-8000-000000000056','056-LIFECYCLE-B-EXPIRE',(select claim_id from lifecycle_b_timeline),(select b_expired_at from lifecycle_b_timeline));
reset role;
select is((select status_code from operations.return_claims where id=(select claim_id from lifecycle_b_timeline)),'EXPIRED','trusted worker expires Claim B after its D14 episode');
insert into lifecycle_fixture_results select 'LIFECYCLE_B_OVERDUE',notification.evaluate_tiktok_claim_deadlines('00000000-0000-4000-8000-000000000056','056-LIFECYCLE-B-OVERDUE',(select b_expired_at from lifecycle_b_timeline),'pgtap.056');
select is((select result->>'status' from lifecycle_fixture_results where kind='LIFECYCLE_B_OVERDUE'),'SUCCEEDED','expired Claim B evaluator rule run succeeds');
select is((select stage_code from notification.notifications where id=(select id from lifecycle_b_episode)),'OVERDUE','Claim B transitions from D14 to OVERDUE after trusted expiry');
select is((select severity_code from notification.notifications where id=(select id from lifecycle_b_episode)),'CRITICAL','OVERDUE stage uses the critical notification severity');
select is((select count(*) from notification.notification_events where notification_id=(select id from lifecycle_b_episode) and event_type_code='STAGE_ESCALATED' and from_stage_code='D14' and to_stage_code='OVERDUE'),1::bigint,'D14 to OVERDUE records one escalation event');
select is((select count(*) from notification.notifications where organization_id='00000000-0000-4000-8000-000000000056'::uuid and entity_id=(select claim_id from lifecycle_b_timeline) and rule_code_snapshot='CLAIM_DEADLINE'),1::bigint,'expiry escalation retains one active notification episode');
select is((select action_route from notification.notifications where id=(select id from lifecycle_b_episode)),'/returns?returnId='||(select return_id from operations.return_claims where id=(select claim_id from lifecycle_b_timeline))||'&claimId='||(select claim_id from lifecycle_b_timeline)||'#claim-detail','expired Claim B keeps its exact return and claim route');
insert into lifecycle_fixture_results select 'LIFECYCLE_B_OVERDUE_RERUN',notification.evaluate_tiktok_claim_deadlines('00000000-0000-4000-8000-000000000056','056-LIFECYCLE-B-OVERDUE-RERUN',(select b_expired_at+interval '1 minute' from lifecycle_b_timeline),'pgtap.056');
select is((select result->>'status' from lifecycle_fixture_results where kind='LIFECYCLE_B_OVERDUE_RERUN'),'SUCCEEDED','overdue evaluator rerun succeeds with unchanged stage');
select is((select stage_code from notification.notifications where id=(select id from lifecycle_b_episode)),'OVERDUE','overdue evaluator rerun keeps OVERDUE stage');
select is((select count(*) from notification.notification_events where notification_id=(select id from lifecycle_b_episode) and event_type_code='STAGE_ESCALATED' and from_stage_code='D14' and to_stage_code='OVERDUE'),1::bigint,'overdue rerun does not append duplicate escalation');
select is((select count(*) from notification.notifications where organization_id='00000000-0000-4000-8000-000000000056'::uuid and entity_id=(select claim_id from lifecycle_b_timeline) and rule_code_snapshot='CLAIM_DEADLINE'),1::bigint,'overdue rerun does not create a second episode');
select is((select jsonb_build_object('transactions',(select coalesce(jsonb_agg(to_jsonb(transaction_row) order by transaction_row.id),'[]'::jsonb) from inventory.stock_transactions transaction_row where transaction_row.organization_id='00000000-0000-4000-8000-000000000056'::uuid),'ledgerEntries',(select coalesce(jsonb_agg(to_jsonb(ledger_row) order by ledger_row.id),'[]'::jsonb) from inventory.stock_ledger_entries ledger_row where ledger_row.organization_id='00000000-0000-4000-8000-000000000056'::uuid),'reservations',(select coalesce(jsonb_agg(to_jsonb(reservation) order by reservation.id),'[]'::jsonb) from inventory.stock_reservations reservation where reservation.organization_id='00000000-0000-4000-8000-000000000056'::uuid),'productPositions',(select coalesce(jsonb_agg(to_jsonb(position) order by position.product_id),'[]'::jsonb) from inventory.stock_product_positions position where position.organization_id='00000000-0000-4000-8000-000000000056'::uuid),'batchPositions',(select coalesce(jsonb_agg(to_jsonb(balance) order by balance.product_id,balance.batch_id),'[]'::jsonb) from inventory.stock_batch_balances balance where balance.organization_id='00000000-0000-4000-8000-000000000056'::uuid))),(select snapshot from lifecycle_b_stock_before),'expiry and overdue evaluation are stock-neutral across transaction, ledger, reservation, product, and batch snapshots');
...
select is((select status_code from operations.return_claims where id=(select claim_id from lifecycle_b_timeline)),'EXPIRED','Claim B acknowledgement does not mutate the expired claim lifecycle');
