        ...
        {selected ? (
          <section className="mt-6 rounded-[var(--ui-radius-lg)] border border-ui-border bg-ui-surface p-5 shadow-[var(--ui-shadow-sm)] sm:p-6">
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div><p className="text-sm font-semibold text-ui-primary">Pemusnahan berhasil dicatat</p><h2 className="mt-1 text-xl font-semibold text-ui-text">{selected.disposal_no}</h2><p className="mt-1 text-sm text-ui-text-muted">{reasonLabel(selected.reason_code_snapshot)} Ã‚Â· {numberFormatter.format(selected.total_quantity)} unit</p></div>
            ...
            {data.lines.length ? <div className="mt-4 grid gap-3">{data.lines.map((line) => <div className="rounded-[var(--ui-radius-md)] border border-ui-border p-4" key={line.disposal_line_id}><p className="font-semibold text-ui-text">{line.product_sku_snapshot} Ã‚Â· Kode Batch {line.batch_code_snapshot}</p><p className="mt-1 text-sm text-ui-text-muted">{conditionLabel(line.source_bucket_code)} Ã‚Â· {numberFormatter.format(line.quantity_disposed)} unit dimusnahkan Ã‚Â· saldo batch {numberFormatter.format(line.bucket_before_qty)} -&gt; {numberFormatter.format(line.bucket_after_qty)} unit</p></div>)}</div> : null}
